...
const EventTypeScreen = ({ navigation, route }) => {
  ...
  console.log(Data, "log data in event type screen");
    ...
    if (item?.is_system_comment) {
      return (
        <View style={styles.systemCommentContainer}>
          <CustomText style={styles.systemCommentText}>{item.comment}</CustomText>
        </View>
      );
    }
    ...
    return (
      <View style={styles.commentContainer}>
        {/* {item.created_by?.profile_picture ? (
          <Image
            source={{ uri: item.created_by.profile_picture }}
            style={styles.avatar}
          />
        ) : ( */}
        <View style={[styles.avatar, { justifyContent: 'center', alignItems: 'center', backgroundColor: mainOrangeColor, left: -10 }]}>
          <CustomText style={{ fontSize: 18, color: mainWhiteColor, fontFamily: fonts.PoppinsMedium }}>
            {`${item.created_by?.first_name?.[0] || ''}${item.created_by?.last_name?.[0] || ''}`.toUpperCase()}
          </CustomText>
        </View>
        {/* )} */}
        <View style={styles.commentTextContainer}>
          <CustomText style={styles.commenterName}>
            {(() => {
          ...
          <CustomText style={styles.commentText}>
            {parts.map((part, index) =>
              part.startsWith('@') ? (
                <CustomText key={index} style={styles.mentionText}>
                  {part}{' '}
                </CustomText>
              ) : (
                part
      ...
      <View style={styles.container}>
        ...
        <View style={styles.contentArea}>
          {Data?.event_type === 'meeting' && (
            <View style={styles.meetingContainer}>
              <View style={styles.header}>
                <CustomText style={styles.meetingTitle}>{MeetingData?.title}</CustomText>
              </View>
              ...
              <View style={[styles.infoRow, styles.spaceBetween]}>
                ...
                <TouchableOpacity
                  ...
                  <BellNotifications
                    ...
                    color={DarkColor80}
                  />
                  <CustomText style={styles.infoText}>{selectedNotification}</CustomText>
          ...
          {Data?.event_type === 'task' && (
            <View style={styles.taskContainer}>
              <View style={styles.taskHeaderRow}>
                <CustomText style={styles.taskTitle}>{Data?.title}</CustomText>
                <TouchableOpacity>
                  {Data?.status === 'completed' || Data?.status === 'task' ? (
              ...
              <CustomText style={styles.taskSubtitle}>{Data?.description}</CustomText>
              ...
              {route?.params?.Data?.status !== 'completed' && (
                <View style={styles.taskButtonRow}>
                  <TouchableOpacity
                    ...
                    <CustomText style={styles.doneButtonText}>Mark as Done</CustomText>
                  </TouchableOpacity>
                  <TouchableOpacity
                    style={styles.rescheduleButton}
                    onPress={() => setRescheduleModalVisible(true)}>
                    <CustomText style={styles.rescheduleButtonText}>Reschedule</CustomText>
                  </TouchableOpacity>
                </View>
              )}
            </View>
          ...
          {Data?.event_type === 'task' && commentStatus && (
            <View>
              <CustomText style={styles.commentsHeaderTitle}>Comments</CustomText>
              <View
          ...
          <View style={styles.modalOverlay}>
            <TouchableWithoutFeedback>
              <View style={styles.modalContent}>
                <CustomText style={styles.modalTitle}>Notification</CustomText>
