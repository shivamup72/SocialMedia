...
const RedFlag = () => <CustomText style={{ fontSize: 18, marginRight: 4 }}>🚩</CustomText>;
const SkyBlueFlag = () => (
  <CustomText style={{ fontSize: 18, marginRight: 4 }}>🟦</CustomText>
);
const CommentIconSvg = () => <CustomText style={{ fontSize: 20 }}>💬</CustomText>;
const PendingDocumentSvg = () => (
  <CustomText style={{ fontSize: 24, color: '#4A4A4A' }}>📋</CustomText>
);
// --------------------------------------------------------------------
...
const PriorityTag = ({ priority }) => {
  if (priority === 'Urgent') {
    return (
      <View style={[styles.priorityTag, { backgroundColor: '#FFD6D6' }]}>
        <RedFlag />
        <CustomText style={[styles.priorityText, { color: '#D9534F' }]}>Urgent</CustomText>
      </View>
    );
  }
  ...
  return (
    <View style={[styles.priorityTag, { backgroundColor: '#D6EFFF' }]}>
      <SkyBlueFlag />
      <CustomText style={[styles.priorityText, { color: '#4A90E2' }]}>Normal</CustomText>
    </View>
  );
};
...
const AssigneeList = ({ assignees }) => {
  ...
  return (
    <View style={styles.assigneeContainer}>
      ...
      {hiddenCount > 0 && (
        <CustomText style={styles.moreUsersText}>+ {hiddenCount} users</CustomText>
      )}
    </View>
  );
};
...
const TaskCard = ({ item }) => {
  return (
    <View style={styles.cardContainer}>
      ...
      <CustomText style={styles.subtasksText}>Subtasks ({item.subtaskCount})</CustomText>

      <View style={styles.cardFooter}>
        <View style={styles.commentsContainer}>
          <CommentIconSvg />
          <CustomText style={styles.commentsText}>{item.commentCount}</CustomText>
        </View>
...
const TaskComponents = () => {
  return (
    <View style={styles.screenContainer}>
      {/* My Tasks Section */}
      <CustomText style={styles.sectionTitle}>My Tasks</CustomText>
      <FlatList
        data={DUMMY_TASKS}
