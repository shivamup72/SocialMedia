    ...
    const DATA = [
        {
            ...
            hubs: workHubs.map((hub: any) => ({
                ...
                offline_users_count: hub.offline_users_count,
                live_users_count: hub.live_users_count,
        ...
        return (
            <View>
                ...
                >
                    ...
                    {/* Arrow icon at the right, toggles expanded state */}
                    <TouchableOpacity onPress={() => setExpandedHubId(isExpanded ? null : item.id)} style={{ width: 30, }}>
                        {isExpanded ? (
                            <DropUpSvg width={16} height={16} color={DarkColor80} />
                        ) : (
                            <DropDownSvgIcon width={"14"} height={"14"} color={DarkColor80} fillOpacity={1} />
                        )}
                    </TouchableOpacity>
                    {/* Existing settings button if selected */}
                    <View>
                        {isSelected && (
                            <TouchableOpacity
                                onPress={() => {
                ...
                {isExpanded && (
                    <View style={{ flexDirection: 'row', justifyContent: 'flex-end', marginRight: 10, marginTop: 4 }}>
                        <CustomText style={{ color: DarkColor, fontSize: 12, marginRight: 12, fontFamily: fonts.PoppinsRegular }}>Offline : {item.offline_users_count}</CustomText>
                        <CustomText style={{ color: DarkColor, fontSize: 12, fontFamily: fonts.PoppinsRegular }}>Online : {item.live_users_count}</CustomText>
                    </View>
                )}
            </View>
        );
