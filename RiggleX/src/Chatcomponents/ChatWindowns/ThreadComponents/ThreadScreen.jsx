...
const ThreadScreen = ({ navigation, route }) => {
  ...
  useEffect(() => {
    ...
    if (
      ...
      console.log('payload check data -=-=-=-=-=-=-=-=-=>', payload, '\n', '\n', lastMessage, '\n', '\n');
      sendMessage(payload);
      loadMessages(page);
      return;
    } else if (
      ...
      setLoading(false);
      // console.log(
      //   'last message action in else if -=-=-=-=-=-=-=-=------->',
      ...
      setDataList(prevData => {
        const messageMap = new Map(prevData.map(msg => [msg.id, { ...msg }]));
        ...
        newMessages.forEach(newMsg => {
          ...
          if (existingMsg) {
            messageMap.set(newMsg.id, {
              ...existingMsg,
              ...newMsg,
              reactions: newMsg?.reactions || existingMsg?.reactions || [],
              status: newMsg?.status || existingMsg?.status || [],
            });
          } else {
            messageMap.set(newMsg.id, {
              ...newMsg,
        ...
        const combinedData = Array.from(messageMap.values()).sort(
      ...
      setHasMore(DataList.length + newMessages.length < totalMessages);
  ...
  const handleRemoveReaction = (message, reactionEmoji) => {
    const payload = {
  ...
  const Header = () => (
    <View style={styles.headerContainer}>
      ...
      {selectedMessageId && (() => {
        const message = DataList.find(item => item.id === selectedMessageId) || route?.params?.Data;
        // Only show InfoSvg if the message was sent by the current user
        if (String(message?.sender?.id) === String(User)) {
          return (
            <TouchableOpacity
    ...
    return (
      <Modal
        ...
        <TouchableOpacity style={styles.modalOverlay} onPress={onClose}>
          <View style={[styles.emojiPickerContainer, { top: positionY }]}>
            ...
            <View style={styles.metaContainer}>
              {item?.reactions?.length > 0 && (
                <View style={styles.reactionsContainer}>
                  ...
                  {item.reactions.length > 2 && (
                    <Pressable onPress={() => handleEmojiModal(item)} android_ripple={{ color: '#eee' }}>
                      <Text style={styles.moreReactions}>+{item.reactions.length - 2}</Text>
                    </Pressable>
                  )}
                </View>
              )}
  ...
  return (
    <KeyboardAvoidingView
      ...
      {EmojiModal && EmojiModalMessage && Array.isArray(EmojiModalData) && EmojiModalData.length > 0 && (
