...
const ScreenWindows = ({ navigation, route }) => {
  ...
  useEffect(() => {
      ...
      if (shouldAddMessage) {
        ...
        setMessages(prevMessages => {
          ...
          const filtered = prevMessages.filter(
            m =>
              !(m.optimistic && m.content === normalizedNewMessage.content && m.isSender)
          );
          // Prevent duplicates
          if (filtered.some(m => m.id === normalizedNewMessage.id)) {
            // console.log('Duplicate message detected, not adding:', normalizedNewMessage.id);
            return filtered;
          }
          const updatedMessages = [normalizedNewMessage, ...filtered];
          // console.log('Updated messages array:', updatedMessages.map(m => m.id));
    ...
    if (
      ...
      // Once the first message is sent for a new conversation, update conversationValue
      // and trigger a full reload to get the correct conversation context.
      if (lastMessage?.data?.conversation_id !== conversationValue) {
  ...
  const handleSend = useCallback(
    async data => {
      ...
      const optimisticMessage = {
        ...
        isSender: true,
        senderName: currentUser?.first_name || 'User name',
        status: [{ status: 'no' }],
        optimistic: true,
        ...
        setMessages(prevMessages =>
          prevMessages.map(msg => {
            if (msg.id === payload.message_id) {
              return {
                ...msg,
                replies_count: (msg.replies_count || 0) + 1, // Increment replies count
              };
            }
            return msg;
          }),
  ...
  // console.log('handleLongPressMessage -=-=-=-------->',currentUser,'\n','\n');
      ...
      setMessages(prevMessages =>
        prevMessages.map(msg =>
          msg.id === selectedMessages[0].id
            ? { ...msg, content: editedMessage, edited: true, updated_at: new Date().toISOString() }
            : msg,
        ),
      );
  ...
  const handleAllDeleteMessage = () => {
    ...
    setHasMore(false);
    // loadMessages();
    // Potentially navigate back or show a "conversation deleted" message
  };
    ...
    <ScreenView>
      ...
      {selectedMessages?.length > 1 && (
        <View style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: '#FFF3E0', padding: 10, zIndex: 100 }}>
          <Text style={{ flex: 1, color: '#FF8C00', fontWeight: 'bold' }}>{selectedMessages.length} selected</Text>
          ...
          <Pressable onPress={() => setSelectedMessages([])} style={{ marginHorizontal: 10 }}>
            <Text style={{ color: '#FF8C00', fontWeight: 'bold' }}>Cancel</Text>
          </Pressable>
        </View>
      )}
      <Windowsheader
