...
const MessageType = ({
  ...
  const showMoreLines = () => {
    setCurrentLines(prevLines =>
      Math.min(prevLines + LINE_STEP, textTotalLines),
    );
  };

  // Function to show fewer lines
  const showLessLines = () => {
    setCurrentLines(prevLines =>
      Math.max(prevLines - LINE_STEP, INITIAL_LINES),
    );
  };
  ...
  return (
    ...
    >
      {item?.is_system_message && !StarListing ? (
          ...
          <Pressable
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center' }}>
              {item?.replies_count > 0 && (
                <TouchableOpacity
                  style={styles.repliesCountContainerSender}
                  onPress={handleThreathNavigation}>
                  {/* my message reply */}
                  <Text
                    allowFontScaling={false}
                    style={[styles.repliesCountText, { marginLeft: 8 }]}>
                    {item?.replies_count}
                  ...
                  {canShowMore && (
                    <TouchableOpacity
                      ...
                      <Text style={styles.readMoreText}>
                        Read More ({textTotalLines - currentLines} lines left)
                      </Text>
                    </TouchableOpacity>
                  )}
                  {canShowLess && (
        ...
        <View style={{ width: '100%', backgroundColor: 'transparent', borderRadius: 10, paddingVertical: 5 }}>
          <View
            style={[
            ...
            <Pressable
              ...
              <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center' }}>
                ...
                {item?.replies_count > 0 && (
                  <TouchableOpacity
                    style={styles.repliesCountContainerReceiver}
                    onPress={handleThreathNavigation}>
