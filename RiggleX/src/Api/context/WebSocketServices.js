...
export const WebSocketProvider = ({children}) => {
  ...
  const sendMessage = useCallback((messageObject, retryCount = 0) => {
    const MAX_RETRIES = 3;
      ...
      if (ws.current?.readyState === WebSocket.OPEN) {
        try {
          ...
          if (retryCount < MAX_RETRIES) {
            ...
            setTimeout(
              () => sendMessage(messageObject, retryCount + 1),
              RETRY_DELAY,
            );
          }
        }
        ...
        if (retryCount < MAX_RETRIES) {
          console.warn(
            `WebSocket not ready. State: ${
              ws.current?.readyState || 'not initialized'
          ...
          console.error('Max retry attempts reached. Could not send message.');
        }
      }
    };
    ...
    if (!ws.current) {
      ...
      // Queue the message for sending after connection
    ...
    // If we have a WebSocket instance, try to send the message
    sendWithRetry();
  }, []);
