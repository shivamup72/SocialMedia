...
const TabNavigation = () => {
  ...
  const ScreenComponent = useMemo(() => {
    ...
    if (activeTab === 1)
      return (
        <ScreenChat hideTabBar={hideTabBar} setHideTabBar={setHideTabBar} />
      );
  ...
  // Toast logic
  useEffect(() => {
    if (activeTab !== 1 && lastMessage?.action === 'receive_new_message') {
      ...
      {toastVisible && (
        <Animated.View
          ...
          <LinearGradient
            ...
            <Text style={styles.snackbarContent}>{toastContent}</Text>
          </LinearGradient>
        </Animated.View>
      )}
